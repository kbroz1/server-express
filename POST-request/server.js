...
function renderWelcomePage(req,res){
    fs.readFile(__dirname + "/index.html", function(err,data){
        if(err){
            ...
            res.end("<html><head><title>Oops</title></head><body><h1>Ooops, there is an error</h1></body")
        }
        else {
            //We then rrespond to the client with the HTML page by specifically telling the browser that we are delivering 
            //an HTML File
